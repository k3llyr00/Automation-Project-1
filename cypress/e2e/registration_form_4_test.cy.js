...
describe('Input fields', () => {
    ...
    it('Username should support letters and numbers, as well as underscores.', () => {
    ...
    it('Email input should support correct pattern', () => {
        // Check regex
        cy.get('input#email').should('have.attr', 'pattern', '[a-z0-9]+@[a-z0-9]+\\.[a-z]{2,4}$');
        // input invalid email
        cy.get('#email').type(' ')
    ...
    it('BMW should not be listed in the list of the cars', () => {
        ...
        // Advanced level how to Check list does not contain BMW
        cy.get('#cars').find('option').then(options => {
            const actual = [...options].map(option => option.text)
            expect(actual).to.not.include('BMW')
        })
